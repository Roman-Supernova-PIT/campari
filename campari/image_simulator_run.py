...
def run_sim(
    ...
    if static_source.lower() == "none":
        static_source = False  # Convert "none" string to actual False boolean for easier handling below
    ...
    if static_source:
        ...
        if any(param is not None for param in [bulge_R, bulge_n, disk_R, disk_n, static_source_mag]):
            raise ValueError("Galaxy parameters provided but static_source is not set to a truthy value")
