...
def generate_diagnostic_plots(fileroot, imsize, plotname, ap_sums=None, ap_err=None, trueflux=None):
    ...
    wcs_all = fits.open(f"/campari_debug_dir/{fileroot}_wcs.fits")
...
def test_noiseless_aligned_no_host():
    ...
    # The 1.651652 comes from the fact that we set an error floor of 1 on each pixel. If we did not, this error
...
def test_sky_noise_aligned_no_host():
    ...
    try:
        ...
        SNLogger.debug("Raising the original exception now.")
        SNLogger.debug(e)
        raise e
