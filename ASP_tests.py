...
def test_calc_mags_and_err():
    ...
    test_mags = np.array([np.nan, 27.66165575,  25.16165575,  22.66165575])
    test_magerr = np.array([np.nan, 1.0857362e-01,
                            1.0857362e-02, 1.0857362e-03])
    test_zp = 15.023547191066587

    assert np.allclose(mags, test_mags, atol=1e-7, equal_nan=True), \
         f"The magnitudes do not match {mags} VS. {test_mags}"
